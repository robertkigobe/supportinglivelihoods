...
<div id="_table_of_contents"><ul><li><a to="#description">Description</a></li><li><a to="#setting-log-file-location">Setting Log File Location</a></li><li><a to="#setting-log-levels">Setting Log Levels</a></li><ul><li><a to="#loglevel"><code>loglevel</code></a></li><ul><li><a to="#aliases">Aliases</a></li></ul><li><a to="#foreground-scripts"><code>foreground-scripts</code></a></li></ul><li><a to="#timing-information">Timing Information</a></li><li><a to="#registry-response-headers">Registry Response Headers</a></li><ul><li><a to="#npm-notice"><code>npm-notice</code></a></li></ul><li><a to="#logs-and-sensitive-information">Logs and Sensitive Information</a></li><li><a to="#see-also">See also</a></li></ul></div>
</section>
...
<p>The default location of the logs directory is a directory named <code>_logs</code> inside the npm cache. This can be changed
with the <code>logs-dir</code> config option.</p>
...
<p>The <code>--timing</code> config can be set which does two things:</p>
<ol>
<li>Always shows the full path to the debug log regardless of command exit status</li>
<li>Write timing information to a timing file in the cache or <code>logs-dir</code></li>
</ol>
<p>This file is a newline delimited list of JSON  objects that can be inspected to see timing data for each task in a <code>npm</code> CLI run.</p>
...
<footer id="edit">
<a to="https://github.com/npm/cli/edit/latest/docs/content/using-npm/logging.md">
